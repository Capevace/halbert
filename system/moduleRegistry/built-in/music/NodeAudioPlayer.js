...
class NodeAudioPlayer {
  ...
  addSource(sources) {
    if (Array.isArray(sources)) {
      this.urlList = [...this.urlList, ...sources];
    } else {
      this.sourceList = [...this.sourceList, sources];
    }
  }
  ...
  play(index) {
    ...
    index = this.currentIndex = index ||
      (this.currentIndex >= this.sourceList.length ? 0 : this.currentIndex);

    const source = this.sourceList[index].replace('https://', 'http://');

    this.decoder = new lame.Decoder().on(
      'format',
      format => this._pipeToSpeaker(format)
    );
  ...
  stop() {
    ...
    if (this.speaker && this.decoder && this.playing) {
      ...
      if (this.httpResponse) this.httpResponse.destroy();

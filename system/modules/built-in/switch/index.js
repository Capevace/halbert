...
module.exports = builder => {
  ...
  builder.widgets
    ...
    .onDataRequest(settings => {
      return {
        switch: find(HALBERT_CONFIG.modules.switches.available, { id: settings.switchId })
      };
    })
    .createWidget('Multi-Switch', 'widget-multi-switch', 'multi-switch.html')
    .onDataRequest(settings => {
      return {
        switches: HALBERT_CONFIG.modules.switches.available.filter(s => settings.switches.includes(s.id))
      };
    });
};

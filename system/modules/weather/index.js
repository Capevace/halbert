...
function onCurrentWeather(result) {
  ...
  fetch(`http://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&APPID=${config.weather.openWeatherMapApiKey}`)
    .then(
      (data) => data.text(),
...
function handleWeather(data, city) {
  ...
  try {
    ...
    console.log(`The current weather in ${city} consists of ${connectingWord} ${weather[0].description}`);
  } catch (e) {
